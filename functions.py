...
def comparar_modelos(knn, arvore):
    ...
    tabela = pd.concat(
        [knn["resultado"], arvore["resultado"]],
        ignore_index=True
    )
...
def relatorio_final(modelos, dados):
    ...
    titulo("RELATÓRIO FINAL - KNN")
    gerar_classification_report(modelos["melhor_knn"], dados)
    gerar_matriz_confusao(modelos["melhor_knn"], dados)

    titulo("RELATÓRIO FINAL - ÁRVORE")
    gerar_classification_report(modelos["melhor_arvore"], dados)
    gerar_matriz_confusao(modelos["melhor_arvore"], dados)

    print("\nVeredito de Negócios:")
    print("...escreva aqui a justificativa conectando FP/FN aos custos reais do banco...")
